...
public func augment(_ V: Volume, crop: Int, dx: Int?, dy: Int?, fliplr: Bool = false) -> Volume {
    // note assumes square outputs of size crop x crop
    let dx = dx ?? Random.Uniform.int(0, V.sx - crop)
    let dy = dy ?? Random.Uniform.int(0, V.sy - crop)
    
    // randomly sample a crop in the input volume
    var W: Volume
    if crop != V.sx || dx != 0 || dy != 0 {
        W = Volume(sx: crop, sy: crop, depth: V.depth, c: 0.0)
        for x in 0 ..< crop {
