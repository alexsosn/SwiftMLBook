...
public class Net {
    ...
    public func getCostLoss(V: inout Volume, y: Int) -> Float {
        _ = forward(&V, isTraining: false)
        let loss = (layers.last! as! LossLayer).backward(y)
        return loss
    }
    
    public func getCostLoss(V: inout Volume, y: Float) -> Float {
        _ = forward(&V, isTraining: false)
    ...
    public func getPrediction() -> Int {
        ...
        guard let outAct = softmax.outAct else {
            fatalError("softmax.outAct is nil.")
        }
